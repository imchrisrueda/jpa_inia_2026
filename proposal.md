...
* **Objetivo**: Detectar y diferenciar la manzana roja real (cultivo) y las bananas (maleza) frente a la cámara web.
* **Flujo principal**:
  1. El expositor coloca la cámara apuntando a una base de demostración.
  2. El usuario coloca una manzana roja real (o una banana/planta de prueba) bajo la cámara.
  3. El sistema identifica automáticamente el objeto en tiempo real, rodeándolo con una retícula de escaneo animada (verde para la manzana/cultivo, roja para la banana/maleza) y mostrando el nombre de la clase e indicador de porcentaje de confianza del modelo.
...
### Caso de Uso 2: Destrucción de Maleza (Acción Láser) - Modo Ratón
* **Persona usuaria**: Visitante del showcase.
* **Objetivo**: Eliminar virtualmente las bananas (malezas) detectadas mediante el "láser de precisión" para proteger el cultivo.
* **Condiciones iniciales**: El selector de modo del HUD está en "MODO RATÓN" (Mouse Mode).
* **Flujo principal**:
  1. El usuario visualiza una banana identificada con retícula roja en el HUD de la pantalla.
  2. El usuario hace clic sobre la banana en la pantalla con el ratón.
...
### Caso de Uso 3: Destrucción de Maleza (Acción Láser) - Modo Auto-disparo
* **Persona usuaria**: Visitante del showcase.
* **Objetivo**: Eliminar virtualmente las malezas mediante fijación y disparo automático.
* **Condiciones iniciales**: El selector de modo del HUD está en "MODO AUTO-DISPARO" (Auto-Spray Mode).
* **Flujo principal**:
  1. El usuario introduce una banana en el encuadre de la cámara.
  2. El sistema detecta y desplaza automáticamente la mira circular del HUD (`#auto-crosshair`) sobre la banana, realizando un seguimiento dinámico si se mueve.
  3. Al mantenerse fijado el objetivo durante 1 segundo, el sistema dispara automáticamente el láser virtual en su posición exacta con efectos de sonido y partículas.
...
* Al mostrar una banana/maleza de prueba, se debe dibujar un cuadro o círculo rojo con animación de "maleza detectada" (*Weed Target*) en menos de 50 ms.
* El selector de modo debe cambiar instantáneamente la lógica de disparo entre clics de ratón y seguimiento automatizado.
* Al hacer clic en un objetivo válido o tras 1 segundo de fijación, debe ejecutarse inmediatamente una animación gráfica de disparo de láser y emitirse un sonido en la posición del objetivo.
* La interfaz debe aplicar consistentemente la paleta del INIA-CSIC (#AF071F y #A2AAAD).

## Supuestos
* Se asume que el PC cuenta con soporte CUDA habilitado y que la GPU RTX 4070Ti permitiría tasas de refresco de vídeo excelentes (>30 FPS) a resolución 1080p.
