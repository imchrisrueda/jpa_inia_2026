...
* **Archivos previstos**:
  ...
  * `model.py` (Gestión del modelo YOLOv8)
...
* **Archivos previstos**:
  * `static/index.html` (Estructura del HUD y contenedor de canvas)
...
* **Criterios de aceptación**:
  ...
  * El frame de la cámara web se pinta en el fondo y se dibujan cajas animadas alrededor de manzanas rojas (verde neón) y dientes de león (rojo CSIC).
...
* **Resultado parcial**: Implementación de las mecánicas de disparo (Modo Ratón y Modo Auto-disparo), marcador de estadísticas de impacto ecológico y efectos visuales de láser.
* **Archivos previstos**:
  * `static/game.js` (Módulo JS con la lógica del juego, colisiones, puntuación y modo de selección)
  * `static/particles.js` (Efectos de chispas e impactos láser)
  * `static/assets/laser.mp3`, `static/assets/explosion.mp3`, `static/assets/error.mp3` (Efectos de sonido)
* **Dependencias**: Tarea 3.
* **Criterios de aceptación**:
  * Se puede cambiar de modo con el control HUD.
  * En modo manual, al hacer clic sobre un recuadro de diente de león se activa la animación de disparo y se destruye el objetivo.
  * En modo automático, al dejar fija la mira central en un diente de león durante 1 segundo, se dispara de forma autónoma.
